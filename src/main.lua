...
-- lanmg
local ScreenTextFile = rom.path.combine(rom.paths.Content, "Game/Text/en/ScreenText.en.sjson") -- SJSON: "Boons Of " // Custom Rarity Display Name
local HelpTextFile = rom.path.combine(rom.paths.Content, "Game/Text/en/HelpText.en.sjson")     -- SJSON: godName, godDescriptionText
local MacroTextFile = rom.path.combine(rom.paths.Content, "Game/Text/en/MacroText.en.sjson")   -- SJSON: Displeased, Sacrifice etc
local CodexTextFile = rom.path.combine(rom.paths.Content, "Game/Text/en/CodexText.en.sjson")   -- God: Codex Descriptions
local TraitTextFile = rom.path.combine(rom.paths.Content, "Game/Text/en/TraitText.en.sjson")   -- SJSON: "Receive your choice of " //  Keepsake: Signoff, Descriptions, Display Name // Boon: Statline, Flavour Text
            ...
            if spawnedItem ~= nil then
                ...
                if not itemData.PendingShopItem and not itemData.ZagContractItem then
                    ...
                    table.insert(CurrentRun.CurrentRoom.Store.SpawnedStoreItems, {
                        ...
                        OriginalResourceCosts = spawnedItem.BaseResourceCosts,
...
function definitions.CreateKeepsake(env, params)
    ...
    if params.customGiftData then
        ...
        if game.GiftData[giftName] == nil then
            ...
            rom.log.warning("GiftData for characterName: " .. giftName .. " already exists, skipping GiftData Creation in customGiftData")
        end
        ...
        if game.LootData[characterName .. "Upgrade"] then
            ...
            for key, _ in pairs(giftTextLines) do
                lastGiftLineKey = key
            end
            ...
            local giftName = "NPC_" .. characterName .. "_01"
            if game.GiftData[giftName] == nil then
                game.GiftData[giftName] = {
                    InheritFrom = { "DefaultGiftData" },
                    MaxedRequirement = {
                ...
                rom.log.warning("GiftData for characterName: " .. giftName .. " already exists, skipping GiftData creation, if this is an issue please report it.")
            end
...
function definitions.CreateBoon(env, params)
    ...
    if params.customStatLine then
        ...
        if params.customStatLine.ID or params.customStatLine.Id or params.customStatLine.id then
            params.customStatLine = { params.customStatLine }
        end

        for _, config in ipairs(params.customStatLine) do
            local statline = sjson.to_object({
                Id = config.ID or config.Id or config.id,
        ...
        if characterData then
            if params.Slot == "Melee" or params.Slot == "Secondary" or params.Slot == "Ranged" or params.Slot == "Rush" or params.Slot == "Mana" then
                ...
                if not alreadyExists then
                    if type(params.addToExistingGod) == "table" and params.addToExistingGod.boonPosition and (params.addToExistingGod.boonPosition < #characterData.WeaponUpgrades) then
                ...
                if not alreadyExists then
                    if type(params.addToExistingGod) == "table" and params.addToExistingGod.boonPosition and (params.addToExistingGod.boonPosition < #characterData.Traits) then
                        table.insert(characterData.Traits, params.addToExistingGod.boonPosition, intboonName)
                    else
...
mods.on_all_mods_loaded(function()
    modutil.once_loaded.game(function()
        for enemyName, enemyData in pairs(EnemyData) do -- dont want to check for name here cause this is just for people adding xyz boons to NPCS
            ...
            if (enemyData.WeaponUpgrades or enemyData.Traits or enemyData.PermanentTraits or enemyData.TemporaryTraits or enemyData.Consumables) ~= nil then
